...
SeaCad through M14.4ar can open an immutable raw ASCII framing document, enforce
...
M14.4n promotes Polyline vertex X/Y to required source-anchored coordinate
semantics and assembles an exact two-component OCS tuple only when both values
are usable. Missing Y is invalid without raw provenance; duplicate, malformed
ASCII, and non-finite Binary states retain the M14.4l issue and available raw
provenance. Tuple failure exposes an exact X/Y unavailable mask, while signed
zero remains bit-exact. The complete M14.4m bulge/header chain remains retained.
...
M14.4q projects one exact-endpoint OCS line result for every M14.4p Straight
segment. Both endpoint tuples must be usable; start and end failures remain
distinct typed outcomes. Arc segments retain their exact bulge instead of
being approximated, and Indeterminate shapes retain the original bulge issue.
Shape classification has precedence over coordinate availability, signed-zero
...
boundary path classified as Edges, then groups every subsequent group-72 field
as an exact source-anchored edge marker. Each marker retains a conservative raw
slice through the next marker or path end, so the final slice may also retain
the path-level source-boundary trailer for later typed partitioning. Declared
and observed edge counts remain Matched or Mismatched without dropping grouped
evidence; zero-edge paths are valid. Polyline paths, unavailable flags, absent,
multiple, malformed, or negative counts, and a marker before the count publish
no edge entries. All nine dialects have ASCII/Binary parity. Edge-type value
semantics, line/circular/elliptic/spline edge fields and geometry, HATCH
applicability, CRUD/write, rendering, and completion remain open.

M14.4u decodes each grouped M14.4t group-72 marker as one signed 16-bit edge
type. Values 1, 2, 3, and 4 publish Line, CircularArc, EllipticArc, and Spline,
respectively. Malformed ASCII remains an InvalidAsciiNumber issue and every
other signed 16-bit value remains ValueOutOfDomain, both anchored to the exact
marker group. Declared/observed count mismatches do not erase independently
known types; empty grouped paths publish an empty typed slice, while Polyline
or otherwise unavailable paths publish no typed entries. All nine dialects
...
M14.4x promotes all four M14.4w components to required source-anchored
coordinate semantics. Four usable finite values assemble exact OCS start and
end points while preserving signed-zero bits. Absent fields become
MissingRequiredValue without raw provenance; duplicate, malformed ASCII, and
non-finite Binary fields retain their original typed numeric issue and any
available raw provenance. An unavailable result reports an exact StartX,
StartY, EndX, and EndY component mask without discarding the complete numeric,
card, type, or grouping chain. Count mismatch does not erase independently
usable endpoints. Valid non-Line, invalid type, and Polyline paths publish no
Line-coordinate entry. All nine dialects have ASCII/Binary parity. OCS/WCS
Line geometry, the other three edge payload families, HATCH applicability,
CRUD/write, rendering, and completion remain open.

M14.4y publishes one exact OCS Line segment per M14.4x coordinate entry when
both OCS endpoints are usable. Start/end values pass through without derived
arithmetic, preserving every signed-zero bit; identical endpoints remain a
valid zero-length Line. Unavailable endpoints remain typed as
EndpointsUnavailable with the complete StartX/StartY/EndX/EndY mask and lower-
layer coordinate issues still resolvable. Count mismatch does not erase usable
Line geometry. Valid non-Line, invalid type, and Polyline paths publish no Line
...
CircularArc: OCS CenterX group 10, CenterY group 20, Radius group 40,
...
does not erase cards; Line, EllipticArc, Spline, invalid, and Polyline states
publish no CircularArc cards while the complete M14.4u directory remains
available. All nine dialects have ASCII/Binary parity. Numeric selection,
required values, radius/angle/direction semantics, OCS/WCS geometry, the other
edge payload families, HATCH applicability, CRUD/write, rendering, and
completion remain open.

M14.4ab selects the six M14.4aa cards into one source-stable numeric entry per
CircularArc edge. Unique finite groups 10/20/40/50/51 become Explicit
binary64 values; unique group 73 becomes an exact signed Int16 without yet
enforcing its Boolean domain. Every explicit or uniquely invalid value retains
...
values and non-finite Binary doubles remain typed issues. Edge-count mismatch
does not erase entries; non-CircularArc, invalid, and Polyline states publish
...
M14.4ah promotes all eight EllipticArc components to required, domain-checked
semantics. Missing fields become source-anchored MissingRequiredValue failures;
duplicate, malformed ASCII, and non-finite Binary values retain their exact
M14.4ag issue and available raw provenance. The two-component OCS major-axis
vector is usable only when its squared magnitude is finite and strictly greater
than `1e-12`; unavailable components, degenerate vectors, and overflowed
derived magnitude remain distinct states. The minor/major ratio accepts the
inclusive `1e-6..=1.0` AcDbEllipse domain, and direction group 73 maps only 0
to Clockwise and 1 to Counterclockwise. Start/end angles remain exact DXF
degrees without normalization. Count mismatch retains entries;
non-EllipticArc, invalid, and Polyline states publish none. All nine dialects
...
M14.4an partitions every grouped HATCH edge payload at the outer path trailer.
Interior edges retain their complete exact payload. For the final edge, the
last group 97 is accepted as the source-boundary-object count only when every
following field is group 330 and no group 330 precedes it; the edge-data range
ends before that anchor and the exact group-97/330 suffix remains separately
available. This preserves an earlier Spline group 97 as its fit-data count,
including fit points and tangents between the two anchors. Missing counts,
references before the outer count, and non-330 suffix fields remain typed
fail-closed results without discarding lower edge/type evidence. All nine
...
exact X anchor and every unique Y/Weight card decode to finite binary64 values
with raw provenance; signed zero remains bit-exact. Missing and duplicate
cards remain distinct, duplicate cards select no raw winner, malformed ASCII
numbers and non-finite Binary doubles remain typed with exact available raw
provenance, and fit points have no inapplicable Weight value. Available empty
sequences publish empty numeric slices, unavailable sequences retain their
lower issue, edge-count mismatch retains usable entries, and all nine dialects
retain ASCII/Binary parity. Required Y semantics, default weight 1, header-to-
observed count relations, knot/tangent cardinality, topology, geometry,
applicability, CRUD/write, rendering, and completion remain open.
