...
use seacad_dxf_core::{
    ...
    DxfBlockAttributeDefinitionPlacementAnchorState, DxfByteSource, DxfCancellationToken,
    DxfDouble, DxfError, DxfMemorySource, DxfReadOptions, DxfResourceProfile, NoopDxfReadObserver,
};
...
fn every_supported_dialect_has_ascii_binary_anchor_selection_parity() -> Result<(), Box<dyn Error>>
{
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = ascii.block_attribute_definition_placement_anchor_directory(
        ...
        let binary_directory = binary.block_attribute_definition_placement_anchor_directory(
            &DxfCancellationToken::default(),
        )?;
...
fn only_the_applicable_tuple_controls_anchor_availability() -> Result<(), Box<dyn Error>> {
    ...
    let directory = document
        .block_attribute_definition_placement_anchor_directory(&DxfCancellationToken::default())?;
    assert_eq!(
        signature(
            directory
...
fn unavailable_justification_precedes_coordinate_selection() -> Result<(), Box<dyn Error>> {
    ...
    let anchor = directory
        .anchor_for_raw_record(directory.records()[0].definition().record().ordinal())?
        .ok_or_else(invalid_test_data)?;
    assert_eq!(
        anchor.state(),
...
fn cancellation_lookups_source_identity_and_public_traits_hold() -> Result<(), Box<dyn Error>> {
    assert_copy::<DxfBlockAttributeDefinitionPlacementAnchor>();
    assert_send_sync::<DxfBlockAttributeDefinitionPlacementAnchorDirectory>();
    ...
    assert_eq!(
        directory
            .anchor_for_block_attribute_definition(block, 0)?
            .map(DxfBlockAttributeDefinitionPlacementAnchor::record),
        Some(first)
    );
    assert!(
        directory
            .anchor_for_block_attribute_definition(block, u64::MAX)?
            .is_none()
    );
    assert!(
        directory
            .anchor_for_block_attribute_definition(u64::MAX, 0)?
            .is_none()
    );
    Ok(())
}
...
fn signatures(
    directory: &DxfBlockAttributeDefinitionPlacementAnchorDirectory,
...
fn signature(
    anchor: DxfBlockAttributeDefinitionPlacementAnchor,
) -> Result<AnchorSignature, DxfError> {
    Ok(match anchor.state() {
        ...
        DxfBlockAttributeDefinitionPlacementAnchorState::AlignmentPointUnavailable => {
            AnchorSignature::AlignmentPointUnavailable
        }
        DxfBlockAttributeDefinitionPlacementAnchorState::TextStart { point } => {
...
fn binary_fixture(version: DxfAcadVersion) -> io::Result<Vec<u8>> {
    ...
    push_definition_header(&mut bytes, version, 0)?;
    for (code, value) in [(10, 1.0), (20, 2.0), (30, 3.0)] {
        push_double(&mut bytes, version, code, value)?;
    }
    push_definition_header(&mut bytes, version, 1)?;
