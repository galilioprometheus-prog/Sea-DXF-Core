...
SeaCad through M4.3b1 can open an immutable raw ASCII framing document, enforce
or recover its EOF envelope, attach a one-pass SHA-256 source identity,
discover an exact HEADER `$ACADVER`, account every parsed group inside or
outside non-overlapping sections, index every numeric group code 0, discover
the exact `$DWGCODEPAGE` declaration, derive a fail-closed text-storage policy,
resolve the reviewed 15-token Windows ANSI subset, decode a complete caller-
provided value into caller-owned UTF-8 without replacement, and write a
separately verified byte-identical copy. The CLI exposes `inspect` and `verify`
with English/Vietnamese human output, JSON v1, stable exits, and path redaction.
These M4 reports and decode primitives remain core APIs and are not exposed in
CLI JSON v1 yet.

| Format | Version | Read | Preserve | Semantic | Edit/Write |
|---|---|---:|---:|---:|---:|
| DXF ASCII | AC1009-AC1032 | Raw framing + dialect/structure/text-storage policy + exact ANSI registry | Verified Verbatim only | Bounded replacement-free UTF-8 primitive; no document text view or record payloads | Not implemented |
...
caller-buffer primitive: document span resolution, escape interpretation,
semantic strings, DOS/OEM pages, and Johab are not implemented. "Verified
