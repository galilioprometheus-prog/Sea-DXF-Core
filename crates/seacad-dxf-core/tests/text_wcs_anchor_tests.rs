...
use seacad_dxf_core::{
    ...
    DxfTextExtrusionComponent, DxfTextOcsPlacementAnchorIssue, DxfTextOcsPlacementAnchorKind,
    DxfTextPlacementComponent, DxfTextSymbolScalarIssue, DxfTextWcsPlacementAnchorDirectory,
    DxfTextWcsPlacementAnchorIssue, DxfTextWcsPlacementAnchorSemantics, NoopDxfReadObserver,
};
...
fn every_dialect_has_ascii_binary_ocs_to_wcs_parity() -> Result<(), Box<dyn Error>> {
    ...
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = ascii.text_wcs_placement_anchor_directory(&cancellation)?;

        let binary_bytes = binary_fixture(version, &rows)?;
        let binary_source = DxfMemorySource::new(&binary_bytes, DxfResourceProfile::Safe)?;
        let binary = open_binary(&binary_source)?;
        let binary_directory = binary.text_wcs_placement_anchor_directory(&cancellation)?;
...
0\nENDSEC\n0\nEOF\n";
    ...
    let directory =
        document.text_wcs_placement_anchor_directory(&DxfCancellationToken::default())?;

    assert_eq!(
        issue(&directory, 0)?,
        DxfTextWcsPlacementAnchorIssue::OcsAnchorInvalid(
...
fn cancellation_scope_identity_lookup_and_public_traits_hold() -> Result<(), Box<dyn Error>> {
    assert_copy::<DxfTextWcsPlacementAnchorSemantics>();
    assert_send_sync::<DxfTextWcsPlacementAnchorDirectory>();
    ...
    cancellation.cancel();
    assert!(matches!(
        document.text_wcs_placement_anchor_directory(&cancellation),
...
0\nSHAPE\n10\n1\n20\n2\n30\n3\n0\nENDSEC\n0\nEOF\n";
    ...
    let directory =
        document.text_wcs_placement_anchor_directory(&DxfCancellationToken::default())?;
    assert_eq!(directory.source_id(), directory.ocs_directory().source_id());
...
fn signatures(directory: &DxfTextWcsPlacementAnchorDirectory) -> Result<Vec<Signature>, DxfError> {
    directory
        .records()
        .iter()
        .copied()
        .filter_map(|record| match directory.semantics_for_record(record) {
            Ok(Some(semantics)) => Some(
                semantics
                    ...
                    .map(|anchor| Signature {
                        kind: anchor.kind(),
                        point: anchor.point().map(canonical_dxf_bits),
                        normal: anchor.normal().map(canonical_dxf_bits),
                    }),
            ),
            Ok(None) => None,
...
fn semantics(
    directory: &DxfTextWcsPlacementAnchorDirectory,
    index: usize,
) -> Result<DxfTextWcsPlacementAnchorSemantics, DxfError> {
    directory
...
fn issue(
    directory: &DxfTextWcsPlacementAnchorDirectory,
    index: usize,
) -> Result<DxfTextWcsPlacementAnchorIssue, DxfError> {
    semantics(directory, index)?
        .wcs_anchor()
        .invalid_issue()
        .copied()
...
fn projected(kind: DxfTextOcsPlacementAnchorKind, point: [f64; 3], normal: [f64; 3]) -> Signature {
    Signature {
        kind,
...
fn binary_fixture(version: DxfAcadVersion, rows: &[Row]) -> io::Result<Vec<u8>> {
    ...
    for row in rows {
        let extrusion = match row {
            ...
            Row::NormalY | Row::SecondNormalY => [0.0, 1.0, 0.0],
...
fn push_text(
    ...
    push_point(bytes, version, [11, 21, 31], [4.0, 5.0, 6.0])?;
    if second {
        push_i16(bytes, version, 72, 1)?;
    }
