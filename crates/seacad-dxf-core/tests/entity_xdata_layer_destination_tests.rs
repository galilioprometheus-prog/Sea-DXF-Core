...
use seacad_dxf_core::{
    ...
    DxfCancellationToken, DxfEntityXDataLayerDestinationDirectory,
    DxfEntityXDataLayerDestinationEntry, DxfEntityXDataLayerDestinationState, DxfError,
...
#[test]
fn every_supported_version_and_format_pair_has_destination_layer_parity()
-> Result<(), Box<dyn Error>> {
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let directories = [
            DxfRawDocumentView::from(&ascii_source).entity_xdata_layer_destination_directory(
            ...
            DxfRawDocumentView::from(&binary_source).entity_xdata_layer_destination_directory(
                DxfRawDocumentView::from(&ascii_destination),
                &token(),
            )?,
            DxfRawDocumentView::from(&binary_source).entity_xdata_layer_destination_directory(
...
#[test]
fn malformed_destination_layer_tables_publish_only_missing() -> Result<(), Box<dyn Error>> {
    let source_bytes =
    ...
    for format in [DxfRawDocumentFormat::Ascii, DxfRawDocumentFormat::Binary] {
        ...
        let states: Vec<_> = match format {
            DxfRawDocumentFormat::Ascii => {
                ...
                source
                    .entity_xdata_layer_destination_directory(
        ...
        assert!(
            states
                .iter()
                .all(|state| { *state == DxfEntityXDataLayerDestinationState::DestinationMissing })
        );
    }
    Ok(())
}
...
-> Result<(), Box<dyn Error>> {
    assert_send_sync::<DxfEntityXDataLayerDestinationDirectory>();
    assert_copy::<DxfEntityXDataLayerDestinationEntry>();
    assert_copy::<DxfEntityXDataLayerDestinationState>();
    let entry_size = size_of::<DxfEntityXDataLayerDestinationEntry>();
    assert!(
        entry_size <= 128,
        "destination LAYER entry is {entry_size} bytes"
    );
    ...
    cancelled.cancel();
    assert!(matches!(
        source.entity_xdata_layer_destination_directory(
            DxfRawDocumentView::from(&destination),
            &cancelled,
        ),
    ...
    let directory = source.entity_xdata_layer_destination_directory(
        DxfRawDocumentView::from(&destination),
        &token(),
    )?;
    ...
    let resolutions = directory.source_resolution_directory();
    let applications = resolutions
        .typed_directory()
        .xdata_directory()
        .applications();
    assert_eq!(directory.entries_for_application(applications[0])?.len(), 7);
    assert_eq!(directory.entries_for_application(applications[1])?.len(), 1);
    assert_eq!(
        directory
    ...
    assert_eq!(
        directory
            .entries_for_entity(applications[1].entity())?
            .len(),
        1
    );
    let debug = format!("{directory:?}");
    assert!(!debug.contains("SECRET_LAYER_OK"));
    assert!(!debug.contains("SECRET_SOURCE_PAYLOAD"));
    assert!(!debug.contains("SECRET_DESTINATION_LAYER"));
    ...
    let other_destination_directory = source.entity_xdata_layer_destination_directory(
        DxfRawDocumentView::from(&other_destination),
        &token(),
    )?;
    let foreign_destination_entry = other_destination_directory.entries()[1];
    assert_eq!(
    ...
    let foreign_resolution = other_source_directory
        .source_resolution_directory()
        .entries()[1];
    let foreign_source_entry = other_source_directory.entries()[1];
    assert_eq!(
        directory.entry_for_source_resolution(foreign_resolution),
        None
    );
    assert_eq!(
    ...
    assert!(matches!(
        directory.entries_for_application(
...
fn assert_directory(directory: &DxfEntityXDataLayerDestinationDirectory) -> Result<(), io::Error> {
    let expected = [3_u8, 0, 1, 2, 3, 4, 1, 0, 0];
        ...
        .enumerate()
    {
        ...
        assert_eq!(resolution.ordinal(), ordinal as u64);
        assert_eq!(
            directory.entry_for_source_resolution(resolution),
            Some(entry)
        );
        assert_eq!(directory.entry(entry.ordinal()), Some(entry));

        let observed = match entry.state() {
            DxfEntityXDataLayerDestinationState::DestinationUnique { target } => {
            ...
            DxfEntityXDataLayerDestinationState::DestinationMissing => 1,
            DxfEntityXDataLayerDestinationState::DestinationAmbiguous { target_count: 2 } => 2,
            DxfEntityXDataLayerDestinationState::SourceMissing => 3,
            DxfEntityXDataLayerDestinationState::SourceAmbiguous { target_count: 2 } => 4,
            _ => return Err(io::Error::other("destination LAYER state")),
        };
...
fn source_fixture(format: DxfRawDocumentFormat, version: DxfAcadVersion) -> io::Result<Vec<u8>> {
    ...
    let groups = document_groups(
        ...
        &[
            ...
            Group(1003, b"ORPHAN"),
            Group(1001, b"APP"),
            Group(1003, b"SECRET_LAYER_OK"),
            Group(1003, b"DEST_MISSING"),
            Group(1003, b"DEST_DUP"),
            Group(1003, b"SOURCE_MISSING"),
            Group(1003, b"SOURCE_DUP"),
            Group(1003, b"NearCase"),
            Group(1003, long_layer.as_slice()),
...
fn destination_fixture(
    ...
    let groups = document_groups(
        ...
        &[
            ...
            Group(0, b"LAYER"),
            Group(2, b"SECRET_LAYER_OK"),
            Group(1000, b"SECRET_DESTINATION_LAYER"),
            Group(0, b"LAYER"),
            Group(2, b"DEST_DUP"),
            Group(0, b"LAYER"),
            Group(2, b"DEST_DUP"),
...
fn malformed_source_fixture(
    ...
    let groups = document_groups(
        ...
        &[
            ...
            Group(0, b"POINT"),
            Group(1001, b"APP"),
            Group(1003, b"UNCLOSED"),
            Group(1003, b"LOWER_TABLE"),
            Group(1003, b"WRONG_TABLE"),
            Group(1003, b"MULTI_NAME"),
            Group(0, b"ENDSEC"),
        ],
    );
    encode(format, version, &groups)
