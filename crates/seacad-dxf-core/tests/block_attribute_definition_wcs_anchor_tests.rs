...
use seacad_dxf_core::{
    ...
    DxfBlockAttributeDefinitionPlacementAnchorState, DxfBlockAttributeDefinitionWcsAnchorDirectory,
...
fn every_supported_dialect_has_ascii_binary_ocs_to_wcs_parity() -> Result<(), Box<dyn Error>> {
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = ascii
            .block_attribute_definition_wcs_anchor_directory(&DxfCancellationToken::default())?;
...
fn unavailable_placement_extrusion_and_zero_normal_fail_typed() -> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(
        issue(&directory, 0)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::PlacementUnavailable(
            DxfBlockAttributeDefinitionPlacementAnchorState::TextStartUnavailable
        )
    );
    assert_eq!(
        issue(&directory, 1)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::ExtrusionUnavailable
    );
    assert_eq!(
        issue(&directory, 2)?,
    ...
    assert_eq!(
        issue(&directory, 3)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::PlacementUnavailable(
            DxfBlockAttributeDefinitionPlacementAnchorState::JustificationUnavailable
        )
    );
    Ok(())
}
...
fn binary_non_finite_inputs_and_derived_overflow_fail_typed() -> Result<(), Box<dyn Error>> {
    ...
    push_definition(&mut bytes, version, [f64::NAN, 2.0, 3.0], [0.0, 0.0, 1.0])?;
    push_definition(
        &mut bytes,
        version,
        [1.0, 2.0, 3.0],
    ...
    assert_eq!(
        issue(&directory, 0)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::NonFinitePlacement
    );
    assert_eq!(
        issue(&directory, 1)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::NonFiniteExtrusion
    );
    assert_eq!(
        issue(&directory, 2)?,
        DxfBlockAttributeDefinitionWcsAnchorIssue::NonFiniteDerivedPoint
    );
    Ok(())
}
...
fn cancellation_lookups_source_identity_and_public_traits_hold() -> Result<(), Box<dyn Error>> {
    assert_copy::<DxfBlockAttributeDefinitionWcsAnchorEntry>();
    assert_send_sync::<DxfBlockAttributeDefinitionWcsAnchorDirectory>();
    ...
    cancellation.cancel();
    assert!(matches!(
        document.block_attribute_definition_wcs_anchor_directory(&cancellation),
    ...
    assert_eq!(
        directory
            .entry_for_block_attribute_definition(block, 0)?
            .map(DxfBlockAttributeDefinitionWcsAnchorEntry::record),
        Some(first)
    );
    assert!(
        directory
            .entry_for_block_attribute_definition(block, u64::MAX)?
            .is_none()
    );
    assert!(
        directory
            .entry_for_block_attribute_definition(u64::MAX, 0)?
            .is_none()
    );
    Ok(())
}
...
fn signatures(
    directory: &DxfBlockAttributeDefinitionWcsAnchorDirectory,
...
fn issue(
    directory: &DxfBlockAttributeDefinitionWcsAnchorDirectory,
    ordinal: usize,
) -> Result<DxfBlockAttributeDefinitionWcsAnchorIssue, DxfError> {
    directory
        .entry_for_definition(directory.records()[ordinal])?
        .ok_or_else(invalid_test_data)?
        .projection()
        .err()
        .ok_or_else(invalid_test_data)
...
fn binary_fixture(version: DxfAcadVersion) -> io::Result<Vec<u8>> {
    let mut bytes = binary_preamble(version)?;
    push_definition(&mut bytes, version, [1.0, 2.0, 3.0], [0.0, 0.0, 1.0])?;
    push_definition(&mut bytes, version, [1.0, 2.0, 3.0], [0.0, 1.0, 0.0])?;
    push_definition(&mut bytes, version, [1.0, 2.0, 3.0], [0.0, 0.0, -2.0])?;
    binary_epilogue(&mut bytes, version)?;
    Ok(bytes)
}
