...
meaning, clone/write, and POINT `Complete` remain open.
M14.3cq resolves every retained XDATA group-1003 layer name against exact
group-2 names from completely closed LAYER tables. Digest indexing bounds the
candidate search while source-span equality remains authoritative; unique,
missing, and ambiguous results stay distinct, including orphan occurrences.
Near-case, wrong-table, malformed-record, and unclosed-table evidence fails
...
M14.3ct resolves every retained XDATA group-1005 handle against the exact
document-local record-identity index. Invalid, null, missing, unique, and
ambiguous results remain distinct; unique results retain the exact target
record and identity candidate, including application values and orphans.
Group-1005-shaped payload inside group-102 application controls cannot enter
the projection. All states have parity across all nine ASCII/Binary dialect
pairs. Handle remap, payload meaning, clone/write, and POINT `Complete` remain
open.
M14.3cu projects caller-supplied cross-document mappings over uniquely resolved
XDATA group-1005 sources. Non-null source/target pairs are copied and sorted;
unmapped, mapped, duplicate-mapping, and every unusable source-resolution state
remain distinct. Only exactly one mapping publishes a destination handle, and
each result is source-bound to prevent ordinal forgery. All states have parity
across all nine ASCII/Binary dialect pairs. Destination identity validation,
payload meaning, clone/write, and POINT `Complete` remain open.
M14.3cv validates each mapped group-1005 destination against the exact identity
index of a separately supplied destination document. Missing, unique, and
ambiguous destination identities remain distinct; remap-unavailable states
propagate unchanged, and only a unique destination exposes its exact record and
identity candidate. Source and destination identities jointly bind every
entry. All four ASCII/Binary source-destination combinations have parity across
all nine dialects. Payload meaning, replacement encoding, clone/write, and
POINT `Complete` remain open.
M14.3cw adds `DxfEntityXDataHandleReplacementDirectory`. Only a uniquely
validated destination becomes replacement-ready, using the canonical group
encoder to publish one complete group 1005 in the destination document's exact
ASCII or Binary dialect. Handles are uppercase, the full 64-bit domain is
retained, and AC1009 Binary uses the required extended-data group-code escape.
...
clone/write, and POINT `Complete` remain open.
M14.3cy adds `DxfEntityXDataHandleReplacementSetDirectory`, grouping every
group-1005 replacement by exact source entity. An entity is `Ready` only when
all of its retained handle values have ready patches; otherwise exact total,
unavailable count, and first unavailable ordinal remain visible. Membership is
source/destination-bound, preventing partial handle remap during later clone or
transaction assembly. Application payload meaning, transaction composition,
clone/write, and POINT `Complete` remain open.
M14.3cz adds same-dialect staging transactions for one complete M14.3cy set.
Every ready group is replaced atomically at its exact full source span and the
transaction captures byte-identical inverse groups. Unavailable sets, format
mismatch, dialect mismatch, foreign evidence, and cancellation produce no
