...
use seacad_dxf_core::{
    DXF_BINARY_SENTINEL, DXF_HATCH_BOUNDARY_CIRCULAR_ARC_EDGE_ROLES, DxfAcadVersion,
    DxfAsciiRawDocument, DxfBinaryRawDocument, DxfByteSource, DxfCancellationToken, DxfError,
    DxfHatchBoundaryCircularArcEdgeCard, DxfHatchBoundaryCircularArcEdgeCardDirectory,
    DxfHatchBoundaryCircularArcEdgeCardState, DxfHatchBoundaryCircularArcEdgeMember,
    DxfHatchBoundaryCircularArcEdgeRole, DxfHatchBoundaryEdgeCountRelation,
    DxfHatchBoundaryEdgePathState, DxfHatchBoundaryEdgeType, DxfMemorySource, DxfReadOptions,
    DxfResourceProfile, NoopDxfReadObserver,
};

#[test]
fn every_dialect_has_ascii_binary_parity_for_six_circular_arc_cards() -> Result<(), Box<dyn Error>>
{
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = open_ascii(&source)?
            .hatch_boundary_circular_arc_edge_card_directory(&DxfCancellationToken::default())?;
        ...
        for directory in [&ascii_directory, &binary_directory] {
            ...
            assert_eq!(directory.members().len(), 6);
            ...
            for (card, role) in directory
                ...
                .iter()
                .copied()
                .zip(DXF_HATCH_BOUNDARY_CIRCULAR_ARC_EDGE_ROLES)
            {
                ...
                let [member] = directory
                    ...
                    .ok_or(io::Error::other("circular arc card members"))?
                else {
                    return Err(io::Error::other("one circular arc card member").into());
                };
                assert_eq!(
                ...
                assert_eq!(
                    directory
                        ...
                        .ok_or(io::Error::other("circular arc edge"))?
                        .edge_type(),
                    Ok(DxfHatchBoundaryEdgeType::CircularArc)
                );
            }
        }
    }
...
#[test]
fn absent_unique_and_multiple_roles_remain_independent() -> Result<(), Box<dyn Error>> {
    ...
    for (role, expected) in [
        (
            ...
            DxfHatchBoundaryCircularArcEdgeCardState::Multiple {
                occurrence_count: 2,
            },
        ),
        (
            DxfHatchBoundaryCircularArcEdgeRole::CenterY,
            DxfHatchBoundaryCircularArcEdgeCardState::Absent,
        ),
        (
        ...
        let card = directory
            ...
            .ok_or(io::Error::other("circular arc role card"))?;
        assert_eq!(card.state(), expected);
        let members = directory
            .members_for_card(card.ordinal())
            .ok_or(io::Error::other("circular arc role members"))?;
...
#[test]
fn mismatch_non_arc_invalid_empty_and_polyline_states_do_not_invent_cards()
-> Result<(), Box<dyn Error>> {
    ...
    assert!(directory.cards_for_edge(3).is_none());
...
fn cancellation_bounds_identity_traits_and_redaction_hold() -> Result<(), Box<dyn Error>> {
    ...
    cancellation.cancel();
    assert!(matches!(
        document.hatch_boundary_circular_arc_edge_card_directory(&cancellation),
    ...
    assert!(size_of::<DxfHatchBoundaryCircularArcEdgeCard>() <= 48);
    assert!(size_of::<DxfHatchBoundaryCircularArcEdgeMember>() <= 96);
    assert!(!format!("{:?}", directory.members()[0]).contains("12345.625"));
    copy::<DxfHatchBoundaryCircularArcEdgeCard>();
    copy::<DxfHatchBoundaryCircularArcEdgeMember>();
    send_sync::<DxfHatchBoundaryCircularArcEdgeCardDirectory>();
    Ok(())
}
