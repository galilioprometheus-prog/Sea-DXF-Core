...
SeaCad through M4.3b2 can open an immutable raw ASCII framing document, enforce
...
UTF-8 without replacement, and write a separately verified byte-identical
copy. Each decode receipt retains source ID, occurrence, raw span, encoding,
and terminal status. The CLI exposes `inspect` and `verify` with
...
|---|---|---:|---:|---:|---:|
| DXF ASCII | AC1009-AC1032 | Raw framing + dialect/structure/text resolution + exact ANSI registry | Verified Verbatim only | Source-anchored caller-buffer UTF-8 group-value view; no escape or record semantics | Not implemented |
...
interpretation, semantic strings, DOS/OEM pages, and Johab are not implemented.
