...
use seacad_dxf_core::{
    DXF_BINARY_SENTINEL, DXF_HATCH_BOUNDARY_ELLIPTIC_ARC_EDGE_ROLES, DxfAcadVersion,
    ...
    DxfHatchBoundaryEllipticArcEdgeCardState, DxfHatchBoundaryEllipticArcEdgeMember,
    DxfHatchBoundaryEllipticArcEdgeRole, DxfMemorySource, DxfReadOptions, DxfResourceProfile,
    NoopDxfReadObserver,
};

#[test]
fn every_dialect_has_ascii_binary_parity_for_eight_elliptic_arc_cards() -> Result<(), Box<dyn Error>>
{
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = open_ascii(&source)?
            .hatch_boundary_elliptic_arc_edge_card_directory(&DxfCancellationToken::default())?;
        ...
        for directory in [&ascii_directory, &binary_directory] {
            ...
            for (card, role) in directory
                ...
                .iter()
                .copied()
                .zip(DXF_HATCH_BOUNDARY_ELLIPTIC_ARC_EDGE_ROLES)
            {
                ...
                let [member] = directory
                    ...
                    .ok_or(io::Error::other("elliptic arc card members"))?
                else {
                    return Err(io::Error::other("one elliptic arc card member").into());
                };
                assert_eq!(
                ...
                assert_eq!(
                    directory
                        ...
                        .ok_or(io::Error::other("elliptic arc edge"))?
                        .edge_type(),
                    Ok(DxfHatchBoundaryEdgeType::EllipticArc)
                );
            }
        }
    }
...
fn absent_unique_and_multiple_roles_remain_independent() -> Result<(), Box<dyn Error>> {
    ...
    for (role, expected) in [
        (
            ...
            DxfHatchBoundaryEllipticArcEdgeCardState::Multiple {
                occurrence_count: 2,
            },
        ),
        (
            DxfHatchBoundaryEllipticArcEdgeRole::CenterY,
            DxfHatchBoundaryEllipticArcEdgeCardState::Absent,
        ),
        (
        ...
        let card = directory
            ...
            .ok_or(io::Error::other("elliptic arc role card"))?;
        assert_eq!(card.state(), expected);
        let members = directory
            .members_for_card(card.ordinal())
            .ok_or(io::Error::other("elliptic arc role members"))?;
...
#[test]
fn mismatch_non_elliptic_invalid_empty_and_polyline_states_do_not_invent_cards()
-> Result<(), Box<dyn Error>> {
    ...
    assert_eq!(directory.cards_for_edge(2), Some(directory.cards()));
    assert_eq!(directory.cards_for_edge(3), Some(&[][..]));
...
fn cancellation_bounds_identity_traits_and_redaction_hold() -> Result<(), Box<dyn Error>> {
    ...
    cancellation.cancel();
    assert!(matches!(
        document.hatch_boundary_elliptic_arc_edge_card_directory(&cancellation),
    ...
    assert!(size_of::<DxfHatchBoundaryEllipticArcEdgeCard>() <= 48);
    assert!(size_of::<DxfHatchBoundaryEllipticArcEdgeMember>() <= 96);
    assert!(!format!("{:?}", directory.members()[0]).contains("12345.625"));
    copy::<DxfHatchBoundaryEllipticArcEdgeCard>();
    copy::<DxfHatchBoundaryEllipticArcEdgeMember>();
    send_sync::<DxfHatchBoundaryEllipticArcEdgeCardDirectory>();
    Ok(())
}
