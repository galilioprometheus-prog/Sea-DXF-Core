...
fn encoding_resolution_cancellation_identity_and_debug_are_explicit() -> Result<(), Box<dyn Error>>
{
    ...
    assert_eq!(plan.encoded_byte_count(), 17);
    let receipt = plan.receipt();
    assert_eq!(receipt.source_id(), plan.source_id());
    assert_eq!(receipt.destination_id(), plan.destination_id());
    assert_eq!(receipt.source_span(), plan.source_span());
    assert_eq!(receipt.source_encoding(), plan.source_encoding());
    assert_eq!(receipt.destination_encoding(), plan.destination_encoding());
    assert_eq!(receipt.source_byte_count(), plan.source_byte_count());
    assert_eq!(receipt.utf8_byte_count(), plan.utf8_byte_count());
    assert_eq!(receipt.encoded_byte_count(), plan.encoded_byte_count());
