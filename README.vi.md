...
Quá trình phát triển đã hoàn tất đến checkpoint **M14.4an**. Core hiện tại có
thể:
...
- chọn numeric semantics hữu hạn, bám source cho đủ bốn tọa độ HATCH Line edge,
  bảo toàn signed zero và giữ typed riêng các trạng thái absent, duplicate,
  ASCII lỗi cùng Binary non-finite;
- nâng đủ bốn tọa độ HATCH Line edge thành semantics bắt buộc và chỉ lắp ráp
  chính xác điểm đầu-cuối OCS khi mọi component khả dụng, nếu không trả mask
  component không khả dụng chính xác;
- cung cấp segment OCS chính xác cho HATCH Line edge bằng cách truyền nguyên
  endpoint khả dụng mà không suy diễn số học, đồng thời giữ typed lỗi endpoint
  và đường thẳng độ dài 0;
- chiếu segment OCS khả dụng của HATCH Line edge sang endpoint WCS hữu hạn cùng
  normal đã normalize bằng elevation, extrusion và arbitrary-axis basis dùng
  chung đã review;
- cung cấp sáu cardinality card chính xác theo role cho mỗi HATCH CircularArc
  edge đã định kiểu: tâm OCS X/Y, bán kính, góc đầu/cuối và cờ ngược chiều kim
  đồng hồ;
- chọn giá trị numeric bám source cho đủ sáu role HATCH CircularArc, giữ năm
...
- bắt buộc đủ năm giá trị header Spline, phân loại chặt cờ rational và periodic
  theo miền 0/1, đồng thời giữ nguyên degree/count signed chính xác mà không tự
  đặt ràng buộc miền chưa được tài liệu hóa;
- phân vùng payload của mọi HATCH edge đã nhóm để edge nội bộ giữ nguyên chính
  xác, còn edge cuối tách phần đuôi group-97/source-object bên ngoài, nhờ đó giữ
  đúng group 97 fit-data của Spline và fail-closed khi thứ tự group 330 sai;
