...
use seacad_dxf_core::{
    ...
    DxfHatchBoundarySplineEdgeHeaderNumericIssue, DxfMemorySource, DxfReadOptions,
    DxfResourceProfile, DxfSemanticValueState, NoopDxfReadObserver,
};

#[test]
fn every_dialect_decodes_exact_ascii_binary_spline_header_widths() -> Result<(), Box<dyn Error>> {
    for version in DxfAcadVersion::SUPPORTED {
        ...
        let ascii_directory = open_ascii(&source)?
            .hatch_boundary_spline_edge_header_numeric_directory(&DxfCancellationToken::default())?;
        let binary = binary_fixture(version, 2, i32::MIN, i16::MIN, i16::MAX, i32::MAX, -1)?;
        ...
        for directory in [&ascii_directory, &binary_directory] {
            let [entry] = directory.entries() else {
                return Err(io::Error::other("one numeric Spline edge").into());
            };
            ...
            assert_eq!(i32_number(components.degree())?, i32::MIN);
            assert_eq!(i16_number(components.rational())?, i16::MIN);
            assert_eq!(i16_number(components.periodic())?, i16::MAX);
            assert_eq!(i32_number(components.knot_count())?, i32::MAX);
            assert_eq!(i32_number(components.control_point_count())?, -1);
            for (value, field_id) in [
                (components.degree(), "boundary_spline_degree"),
                (components.knot_count(), "boundary_spline_knot_count"),
                (
                    components.control_point_count(),
            ...
            for (value, field_id) in [
                (components.rational(), "boundary_spline_rational"),
                (components.periodic(), "boundary_spline_periodic"),
            ] {
                assert_eq!(value.state(), DxfSemanticValueState::Explicit);
                assert_eq!(value.field_provenance().schema_namespace(), "entity.hatch");
                assert_eq!(value.field_provenance().schema_field_id(), field_id);
...
#[test]
fn absent_multiple_and_malformed_i16_i32_values_remain_distinct() -> Result<(), Box<dyn Error>> {
    let ascii = ascii_fixture(
    ...
    let entry = directory
        .entry(0)
        .ok_or_else(|| io::Error::other("numeric Spline header entry"))?;
    let components = entry.components();
    assert!(matches!(
        components.degree().invalid_issue(),
        Some(
            DxfHatchBoundarySplineEdgeHeaderNumericIssue::InvalidAsciiNumber(
                DxfAsciiNumericIssue::InvalidSyntax { .. }
            )
        )
    ));
    assert!(components.degree().raw_provenance().is_some());
    assert_eq!(
        components.rational().invalid_issue(),
        Some(
            &DxfHatchBoundarySplineEdgeHeaderNumericIssue::MultipleValues {
                occurrence_count: 2
            }
        )
    );
    assert_eq!(components.rational().raw_provenance(), None);
    assert_eq!(components.periodic().state(), DxfSemanticValueState::Absent);
    assert_eq!(components.periodic().raw_provenance(), None);
    assert!(matches!(
        components.knot_count().invalid_issue(),
        Some(
            DxfHatchBoundarySplineEdgeHeaderNumericIssue::InvalidAsciiNumber(
                DxfAsciiNumericIssue::InvalidSyntax { .. }
            )
    ...
    assert!(components.knot_count().raw_provenance().is_some());
    assert_eq!(i32_number(components.control_point_count())?, 4);
    Ok(())
}

#[test]
fn count_mismatch_signed_domains_and_non_spline_paths_remain_fail_closed()
-> Result<(), Box<dyn Error>> {
    let binary = binary_fixture(DxfAcadVersion::Ac1032, 0, -7, -3, 2, -4, 0)?;
    let source = DxfMemorySource::new(&binary, DxfResourceProfile::Safe)?;
    let directory = open_binary(&source)?
        .hatch_boundary_spline_edge_header_numeric_directory(&DxfCancellationToken::default())?;
    let entry = directory
        .entry_for_edge(1)
        .ok_or_else(|| io::Error::other("count-mismatched Spline entry"))?;
    assert_eq!(i32_number(entry.components().degree())?, -7);
    assert_eq!(i16_number(entry.components().rational())?, -3);
    assert_eq!(i16_number(entry.components().periodic())?, 2);
    assert_eq!(i32_number(entry.components().knot_count())?, -4);
    assert_eq!(i32_number(entry.components().control_point_count())?, 0);
...
fn cancellation_bounds_identity_traits_and_redaction_hold() -> Result<(), Box<dyn Error>> {
    ...
    assert!(size_of::<DxfHatchBoundarySplineEdgeHeaderNumericComponents>() <= 640);
    assert!(size_of::<DxfHatchBoundarySplineEdgeHeaderNumericEntry>() <= 656);
    let debug = format!("{:?}", directory.entries()[0].components().degree());
    assert!(!debug.contains("123456789"));
    copy::<DxfHatchBoundarySplineEdgeHeaderNumericComponents>();
    copy::<DxfHatchBoundarySplineEdgeHeaderNumericEntry>();
    send_sync::<DxfHatchBoundarySplineEdgeHeaderNumericDirectory>();
    Ok(())
}

fn i16_number(
    value: &seacad_dxf_core::DxfHatchBoundarySplineEdgeHeaderNumericI16Value,
) -> Result<i16, io::Error> {
    value
        .value()
        .copied()
        .ok_or_else(|| io::Error::other("explicit i16"))
}

fn i32_number(
    value: &seacad_dxf_core::DxfHatchBoundarySplineEdgeHeaderNumericI32Value,
) -> Result<i32, io::Error> {
    value
        .value()
        .copied()
        .ok_or_else(|| io::Error::other("explicit i32"))
}
