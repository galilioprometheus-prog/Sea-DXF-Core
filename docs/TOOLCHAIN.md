...
shell with a process-local execution-policy bypass for Actions-generated
temporary scripts, avoiding both a machine policy change and an unreviewed
PowerShell 7 installation. Six-platform hosted execution is reserved for the
separate manual Native Release Artifacts workflow, which builds and tests all
six native targets and aggregates their exact receipts. The manual standalone
